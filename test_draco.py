...
#     res = fmdt.detect(vid, light_max=255, light_min=int(x), out_track_file="draco_tmp.txt", log=True)
...
for lm in np.linspace(50, 220, 10):
    res = fmdt.detect(vid, light_max=225, light_min=int(lm), out_track_file="draco_tmp.txt", log=True)
    print(res.tracking_list)
